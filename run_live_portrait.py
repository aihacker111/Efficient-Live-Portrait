...
def main(video_path, source_img, source_video_path, ref_img, max_faces, use_tensorrt, task, half_precision, use_face_id, cropping_video, get_source_audio):
    ...
    if use_face_id:
        live_portrait.render(video_path_or_id=video_path, image_path=source_img, source_video_path=source_video_path, ref_img=ref_img, max_faces=max_faces, task=task, audio_from_source=get_source_audio)
    else:
        live_portrait.render(video_path_or_id=video_path, image_path=source_img, source_video_path=source_video_path, ref_img=None, max_faces=max_faces, task=task, audio_from_source=get_source_audio)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='image', help='Options for real-time webcam or source_image or source_video demo')
    ...
    parser.add_argument('--get_source_audio', action='store_true', required=False, help='If you want to get the output with audio from source video')
