...
def main(video_path, source_img, ref_img, max_faces, use_tensorrt, real_time, half_precision, use_face_id, cropping_video):
    ...
    if use_face_id:
        live_portrait.render(video_path_or_id=video_path, image_path=source_img, ref_img=ref_img, max_faces=max_faces, real_time=real_time, cropping_video=cropping_video)
    else:
        live_portrait.render(video_path_or_id=video_path, image_path=source_img, ref_img=None, max_faces=max_faces, real_time=real_time, cropping_video=cropping_video)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cropping_video', action='store_true', help='Auto Cropping 1:1 Video')
